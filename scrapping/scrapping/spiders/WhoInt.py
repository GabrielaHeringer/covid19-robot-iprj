...
class WhoIntSpider(scrapy.Spider):
	...
	def parse(self, response):
		...
		search_page = "https://search.bvsalud.org/global-literature-on-novel-coronavirus-2019-ncov/"
	...
	def fix_ris_data(self, text_lines):
		data = list(map(lambda x: x if x != 'ER  -' else 'ER  - ', text_lines))

		if data[0][0] != 'T' and data[0][1] != 'Y':
		    data.insert(0, 'TY  - JOUR')

		for i in range(1, (len(data)-1)):
		    if data[i-1] == 'ER  - ' and data[i+1][0] != 'T' and data[i+1][1] != 'Y':
		        data.insert(i, 'TY  - JOUR')
		return data
